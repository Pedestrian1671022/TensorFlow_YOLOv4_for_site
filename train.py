...
# 训练
def backward():
    yolov4 = YOLO(config.class_num, config.anchors, width=config.width, height=config.height)
    dataset = Data(config.train_file, config.class_num, config.batch_size, config.anchors, config.multi_scale_img, width=config.width, height=config.height)
    ...
    feature_y1, feature_y2, feature_y3 = yolov4.forward(inputs, weight_decay=config.weight_decay, isTrain=True)

    global_step = tf.Variable(0, trainable=False)
    
    # 损失 yolov4
    loss = yolov4.get_loss_v4(feature_y1, feature_y2, feature_y3, y1_true, y2_true, y3_true, 
    ...
    epoch = compute_curr_epoch(global_step, config.batch_size, len(dataset.imgs_path))
    ...
    with tf.compat.v1.Session() as sess:
        ...
        # 一共迭代这么多次
        total_steps = np.ceil(config.total_epoch * len(dataset.imgs_path) / config.batch_size)
        while step < total_steps:
            start = time.perf_counter()
            batch_img, y1, y2, y3 = next(dataset)
            ...
            print("step: %6d, loss: %.5g\t, w: %3d, h: %3d, lr:%.5g\t, time: %5f s"
                         %(step, loss_, dataset.width, dataset.height, lr_, end-start))
