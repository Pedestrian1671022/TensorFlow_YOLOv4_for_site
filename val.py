...
def save_img(img, name):
    ...
    img_name = os.path.join(config.save_dir, name)
    cv2.imwrite(img_name, img)
    return 


def main():
    yolov4 = YOLO(config.class_num, config.anchors)

    inputs = tf.compat.v1.placeholder(dtype=tf.float32, shape=[1, None, None, 3])
    feature_y1, feature_y2, feature_y3 = yolov4.forward(inputs, isTrain=False)
    pre_boxes, pre_score, pre_label = yolov4.get_predict_result(feature_y1, feature_y2, feature_y3, config.class_num, 
    ...
    with tf.compat.v1.Session() as sess:
        ...
        for name in os.listdir(config.val_dir):
            img_name = os.path.join(config.val_dir, name)
